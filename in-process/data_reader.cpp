...
#include "../src/internals/internal_array.hpp"
#include "../src/internals/internal_tensor.hpp"
    ...
    void read_features(std::string filepath) {
        ...
        if (file.is_open()) {
            ...
            for (std::size_t batch = 0; batch < num_imgs; batch += batch_size_) {
                features_.emplace_back(Tensor<float>({batch_size_, features_size_}));
                for (std::size_t index = 0; index < batch_size_; ++index) {
                    std::vector<uint8_t> features(features_size_);
                    file.read(reinterpret_cast<char*>(&features[0]), features_size_);
                    std::transform(features.begin(), features.end(), features_.back().data() + index * features_size_, [](uint8_t x) { return static_cast<float>(x) / 255.0f; });
                }
